// Package updatewire builds the tuikit UpdateConfig used by cryptstream.
...
// New returns a UpdateConfig wired for the cryptstream binary. Callers
// pass the current version string (typically set via ldflags). The mode
// is UpdateForced so release notes carrying a `minimum_version:` marker
